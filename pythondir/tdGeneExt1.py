...
'''create patches from patient on front view, including ROI when overlapping
        ...
        sylvain kritter 07 april 2017
    includes dicom treatment from kaggkle
 '''
import os
import cv2
#import dircache
...
def genepara(namedirtopcf): 
    dirFileP = os.path.join(namedirtopcf, 'source')
        #list dcm files
#    print dirFileP, os.listdir(dirFileP)
#    slices = [dicom.read_file(os.path.join(dirFileP, s)) for s in os.listdir(dirFileP) if ".dcm" in s.lower()]
...
##    print slices
#    print 'slice_thickness new',slice_thickness
...
#                print 'dsr orig', dsr.min(),dsr.max()
                if dsr.max()>0:
                    if sou !='source' :
                        dsr= dsr-dsr.min() 
                        c=float(255)/dsr.max()
...
#                        dsr = dsr.astype('int16')
                        ...
                        dsr = dsr.astype('int16')
#                        print 'dsr HU', dsr.min(),dsr.max()
                        dsr=normalize(dsr)
#                        print 'dsr normalized', dsr.min(),dsr.max()
                        dsr=zero_center(dsr)
#                        print 'dsr enter', dsr.min(),dsr.max()
#                        print dsr.min(),dsr.max()
#                        plt.hist(dsr.flatten(), bins=80,color='c')
...
#                    dsrresize = scipy.ndimage.interpolation.zoom(dsr, fxs, mode='nearest')
                    ...
                    dsrresize=cv2.resize(np.array(dsr),None,fx=fxs,fy=fxs,interpolation=cv2.INTER_LINEAR)
...
#        print 'dimtabxd', dimtabxd
        ...
        print 'tabscan',tabscan.shape
#        p = tabscan.transpose(2,0,1)
#        print 'p',p.shape
