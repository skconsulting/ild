...
def prediction(X_test, y_test, params,pfile):
    f=open (pfile+'/res.txt','w')
    pfileh5=os.path.join(pfile,'ILD_CNN_model.h5')
#    model = H.load_model(pfileh5)
    model = load_model(pfileh5)
...
#    y_predict = np.argmax(y_test_subset)
    ...
    for i in range (0,n):
        ...
        f.write('\n')
    f.close()
    open(pfile+ '/TestLog.csv', 'a').write(str(params['res_alias']) + ', ' + str(str(fscore) + ', ' + str(acc)+'\n'))
    return